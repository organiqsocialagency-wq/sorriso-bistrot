...
if(slides.length){
 ...
 slides.forEach((s,i)=>{const v=s.querySelector('video');if(videoSources[i]){v.src=videoSources[i];v.hidden=false;s.querySelector('.video-placeholder').hidden=true;s.classList.add('has-video');v.addEventListener('play',()=>{paused=true;updateButton();schedule()});v.addEventListener('error',()=>{v.hidden=true;s.classList.remove('has-video');s.querySelector('.video-placeholder').hidden=false})}});
...
if(form){
 ...
 function service(value){form.elements.servizio.value=value;document.querySelectorAll('[data-service]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.service===value)));document.querySelector('#people-field').hidden=value==='asporto';form.elements.persone.disabled=value==='asporto';form.elements.note.required=value==='asporto';document.querySelector('#note-label').textContent=value==='asporto'?'Il tuo ordine: prodotti, quantità e impasti':'Vuoi aggiungere qualcosa? (facoltativo)';form.elements.note.placeholder=value==='asporto'?'Es. 2 Margherite senza glutine e 2 supplì':'Un’occasione speciale, una preferenza…';reset()}
...
(()=>{
 ...
 const marquee=document.querySelector('.reviews-marquee');if(marquee){const row=marquee.querySelector('.reviews-row'),track=row.querySelector('.reviews-track');const originals=[...track.children];originals.forEach(c=>{const copy=c.cloneNode(true);copy.setAttribute('aria-hidden','true');copy.inert=true;track.append(copy)});const second=row.cloneNode(true);second.setAttribute('aria-hidden','true');second.inert=true;marquee.append(second);const toggle=document.querySelector('#review-toggle');toggle.addEventListener('click',()=>{const paused=marquee.classList.toggle('paused');toggle.setAttribute('aria-pressed',String(paused));toggle.textContent=paused?'Riprendi scorrimento':'Pausa scorrimento'});document.addEventListener('visibilitychange',()=>{marquee.classList.toggle('page-hidden',document.hidden)})}
})();
